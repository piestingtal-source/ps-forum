...
class PSF_Default extends PSF_Theme_Compat {
	...
	public function __construct( $properties = array() ) {
		...
		parent::__construct( psf_parse_args( $properties, array(
			'id'      => 'default',
			'name'    => __( 'PS Forum Standard', 'psforum' ),
	...
	private function setup_actions() {
		...
		add_action( 'psf_before_main_content',  array( $this, 'before_main_content'   ) ); // Top wrapper HTML
		add_action( 'psf_after_main_content',   array( $this, 'after_main_content'    ) ); // Bottom wrapper HTML
	...
	public function localize_topic_script() {
		...
		if ( psf_is_single_forum() ) {
			wp_localize_script( 'psforum-forum', 'psfForumJS', array(
				...
				'generic_ajax_error' => __( 'Etwas ist schief gelaufen. Aktualisiere Deinen Browser und versuche es erneut.', 'psforum' ),
	...
	public function ajax_forum_subscription() {
		...
		if ( ! psf_is_subscriptions_active() ) {
			psf_ajax_response( false, __( 'Abonnements sind nicht mehr aktiv.', 'psforum' ), 300 );
		}

		// Bail if user is not logged in
		if ( ! is_user_logged_in() ) {
			psf_ajax_response( false, __( 'Bitte logge Dich sich ein, um dieses Forum zu abonnieren.', 'psforum' ), 301 );
		}
		...
		if ( ! current_user_can( 'edit_user', $user_id ) ) {
			psf_ajax_response( false, __( 'Du hast keine Erlaubnis das zu tun.', 'psforum' ), 302 );
		}
		...
		if ( empty( $forum ) ) {
			psf_ajax_response( false, __( 'Das Forum konnte nicht gefunden werden.', 'psforum' ), 303 );
		}

		// Bail if user did not take this action
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'toggle-subscription_' . $forum->ID ) ) {
			psf_ajax_response( false, __( 'Bist Du sicher, dass Du das tun willst?', 'psforum' ), 304 );
		}
		...
		if ( empty( $status ) ) {
			psf_ajax_response( false, __( 'Die Anfrage war erfolglos. Bitte versuche es erneut.', 'psforum' ), 305 );
		}
	...
	public function ajax_favorite() {
		...
		if ( ! psf_is_favorites_active() ) {
			psf_ajax_response( false, __( 'Favoriten sind nicht mehr aktiv.', 'psforum' ), 300 );
		}

		// Bail if user is not logged in
		if ( ! is_user_logged_in() ) {
			psf_ajax_response( false, __( 'Bitte logge Dich ein, um dieses Thema zu einem Favoriten zu machen.', 'psforum' ), 301 );
		}
		...
		if ( empty( $topic ) ) {
			psf_ajax_response( false, __( 'Das Thema konnte nicht gefunden werden.', 'psforum' ), 303 );
		}
	...
	public function ajax_subscription() {
		...
		// Bail if user is not logged in
		if ( ! is_user_logged_in() ) {
			psf_ajax_response( false, __( 'Bitte logge Dich ein, um dieses Thema zu abonnieren.', 'psforum' ), 301 );
		}
