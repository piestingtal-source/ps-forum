 ...
 * @since PSForum (r2593)
 *
 * @uses apply_filters() Calls 'psf_get_reply_caps' with the capabilities
 * @return array Reply capabilities
 */
function psf_get_reply_caps() {
	return apply_filters( 'psf_get_reply_caps', array (
 ...
 */
function psf_map_reply_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
	...
	switch ( $cap ) {
		...
		case 'psf_replies_admin' :
			$caps = array( 'moderate' );
			break;
	}

	return apply_filters( 'psf_map_reply_meta_caps', $caps, $cap, $user_id, $args );
}
