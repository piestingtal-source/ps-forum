...
class PSF_Akismet {
	...
	/**
	 * The main PSForum Akismet loader
	 *
	 * @since PSForum (r3277)
	 *
	 * @uses add_filter()
	...
	private function setup_actions() {
		...
		// PSForum functions to check for spam
		$checks['check']  = array(
			'psf_new_topic_pre_insert'  => 1,  // New topic check
			'psf_new_reply_pre_insert'  => 1,  // New reply check
			'psf_edit_topic_pre_insert' => 1,  // Edit topic check
			'psf_edit_reply_pre_insert' => 1   // Edit reply check
		);

		// PSForum functions for spam and ham submissions
		$checks['submit'] = array(
			'psf_spammed_topic'   => 10, // Spammed topic
			'psf_unspammed_topic' => 10, // Unspammed reply
			'psf_spammed_reply'   => 10, // Spammed reply
			'psf_unspammed_reply' => 10, // Unspammed reply
		);
	 ...
	 * @uses psf_filter_anonymous_user_data() To get anonymous user data
	 * @uses psf_get_topic_permalink() To get the permalink of the topic
	 * @uses psf_get_reply_url() To get the permalink of the reply
	 * @uses psf_current_author_ip() To get the IP address of the current user
	 * @uses PSF_Akismet::maybe_spam() To check if post is spam
	 * @uses PSF_Akismet::get_user_roles() To get the role(s) of the current user
	 * @uses do_action() To call the 'psf_akismet_spam_caught' hook
	 * @uses add_filter() To call the 'psf_new_reply_pre_set_terms' hook
	 *
	...
	public function check_post( $post_data ) {
		...
		if ( psf_get_public_status_id() !== $post_data['post_status'] )
			return $post_data;
		...
		if ( !empty( $anonymous_data ) ) {
			$user_data['name']    = $anonymous_data['psf_anonymous_name'];
			$user_data['email']   = $anonymous_data['psf_anonymous_email'];
			$user_data['website'] = $anonymous_data['psf_anonymous_website'];
		...
		// Get the result
		$post_data['psf_akismet_result'] = $_post['psf_akismet_result'];
		unset( $_post['psf_akismet_result'] );

		// Store the data as submitted
		$post_data['psf_post_as_submitted'] = $_post;

		// Allow post_data to be manipulated
		do_action_ref_array( 'psf_akismet_check_post', $post_data );

		// Spam
		if ( 'true' === $post_data['psf_akismet_result'] ) {

			// Let plugins do their thing
			do_action( 'psf_akismet_spam_caught' );

			// This is spam
			$post_data['post_status'] = psf_get_spam_status_id();

			// We don't want your spam tags here
			add_filter( 'psf_new_reply_pre_set_terms', array( $this, 'filter_post_terms' ), 1, 3 );
	 ...
	 * @uses psf_get_user_profile_url() To get a user's profile url
	 * @uses get_permalink() To get the permalink of the post_parent
	 * @uses PSF_Akismet::get_user_roles() To get the role(s) of the post_author
	 * @uses psf_current_author_ip() To get the IP address of the current user
	 * @uses PSF_Akismet::maybe_spam() To submit the post as ham or spam
	 * @uses update_post_meta() To update the post meta with some Akismet data
	 * @uses do_action() To call the 'psf_akismet_submit_spam_post' and 'psf_akismet_submit_ham_post' hooks
	 *
	...
	public function submit_post( $post_id = 0 ) {
		...
		switch ( $current_filter ) {
			...
			case 'psf_spammed_topic' :
			case 'psf_spammed_reply' :
				$request_type = 'spam';
				break;

			// Honey-glazed, a straight off the bone
			case 'psf_unspammed_topic' :
			case 'psf_unspammed_reply' :
				$request_type = 'ham';
				break;
		...
		if ( ( 'spam' === $request_type ) && ( psf_get_spam_status_id() !== $_post->post_status ) )
			return;

		// Set some default post_data
		$post_data = array(
			'comment_approved'     => $_post->post_status,
			'comment_author'       => $_post->post_author ? get_the_author_meta( 'display_name', $_post->post_author ) : get_post_meta( $post_id, '_psf_anonymous_name',    true ),
			'comment_author_email' => $_post->post_author ? get_the_author_meta( 'email',        $_post->post_author ) : get_post_meta( $post_id, '_psf_anonymous_email',   true ),
			'comment_author_url'   => $_post->post_author ? psf_get_user_profile_url(            $_post->post_author ) : get_post_meta( $post_id, '_psf_anonymous_website', true ),
		...
		if ( isset( $post_data['reporter'] ) ) {
			...
			switch ( $request_type ) {
				...
				// Spammy
				case 'spam' :
					$this->update_post_history( $post_id, sprintf( esc_html__( '%1$s reported this %2$s as spam', 'psforum' ),     $post_data['reporter'], $post_data['comment_type'] ), 'report-spam' );
					update_post_meta( $post_id, '_psf_akismet_user_result', 'true'                 );
					update_post_meta( $post_id, '_psf_akismet_user',        $post_data['reporter'] );
					break;

				// Hammy
				case 'ham'  :
					$this->update_post_history( $post_id, sprintf( esc_html__( '%1$s reported this %2$s as not spam', 'psforum' ), $post_data['reporter'], $post_data['comment_type'] ), 'report-ham'  );
					update_post_meta( $post_id, '_psf_akismet_user_result', 'false'                 );
					update_post_meta( $post_id, '_psf_akismet_user',         $post_data['reporter'] );
		...
		do_action( 'psf_akismet_submit_' . $request_type . '_post', $post_id, $post_data['psf_akismet_result'] );
	}
	...
	private function maybe_spam( $post_data, $check = 'check', $spam = 'spam' ) {
		...
		if ( !empty( $response[1] ) ) {
			$post_data['psf_akismet_result'] = $response[1];
		} else {
			$post_data['psf_akismet_result'] = esc_html__( 'No response', 'psforum' );
		}
	 ...
	 * @uses PSF_Akismet::update_post_history() To update post Akismet history
	 */
	public function update_post_meta( $post_id = 0, $_post = false ) {
		...
		if ( !empty( $this->last_post ) )
			$as_submitted = $this->last_post['psf_post_as_submitted'];

		// wp_insert_post() might be called in other contexts. Ensure this is
		// the same topic/reply as was checked by PSF_Akismet::check_post()
		if ( is_object( $_post ) && !empty( $this->last_post ) && is_array( $as_submitted ) ) {
			...
			if (	intval( $as_submitted['comment_post_ID'] )    === intval( $_post->post_parent )
					&&      $as_submitted['comment_author']       === ( $anonymous_data ? $anonymous_data['psf_anonymous_name']  : $userdata->display_name )
					&&      $as_submitted['comment_author_email'] === ( $anonymous_data ? $anonymous_data['psf_anonymous_email'] : $userdata->user_email   )
				) {

				// Normal result: true
				if ( $this->last_post['psf_akismet_result'] === 'true' ) {

					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_psf_akismet_result', 'true' );
					$this->update_post_history( $post_id, esc_html__( 'Akismet caught this post as spam', 'psforum' ), 'check-spam' );

					// If post_status isn't the spam status, as expected, leave a note
					if ( psf_get_spam_status_id() !== $_post->post_status ) {
						$this->update_post_history( $post_id, sprintf( esc_html__( 'Post status was changed to %s', 'psforum' ), $_post->post_status ), 'status-changed-' . $_post->post_status );
					}

				// Normal result: false
				} elseif ( $this->last_post['psf_akismet_result'] === 'false' ) {

					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_psf_akismet_result', 'false' );
					$this->update_post_history( $post_id, esc_html__( 'Akismet cleared this post as not spam', 'psforum' ), 'check-ham' );
				...
				// Abnormal result: error
					...
					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_psf_akismet_error', time() );
					$this->update_post_history( $post_id, sprintf( esc_html__( 'Akismet was unable to check this post (response: %s), will automatically retry again later.', 'psforum' ), $this->last_post['psf_akismet_result'] ), 'check-error' );
				}

				// Record the complete original data as submitted for checking
				if ( isset( $this->last_post['psf_post_as_submitted'] ) ) {
					update_post_meta( $post_id, '_psf_akismet_as_submitted', $this->last_post['psf_post_as_submitted'] );
				}
			}
		}
	}
	...
	private function update_post_history( $post_id = 0, $message = null, $event = null ) {
		...
		// Save the event data
		add_post_meta( $post_id, '_psf_akismet_history', $event );
	}
	...
	public function get_post_history( $post_id = 0 ) {
		...
		$history = get_post_meta( $post_id, '_psf_akismet_history' );
	 ...
	 * @param int $reply_id
	 *
	 * @uses psf_get_reply_id() To get the reply_id
	 * @uses psf_get_topic_id() To get the topic_id
	...
	public function filter_post_terms( $terms = '', $topic_id = 0, $reply_id = 0 ) {
		...
		$existing_terms = wp_get_object_terms( $topic_id, psf_get_topic_tag_tax_id(), array( 'fields' => 'names' ) );

		// Save the terms for later in case the reply gets hammed
		if ( !empty( $terms ) )
			update_post_meta( $reply_id, '_psf_akismet_spam_terms', $terms );
	...
	/**
	 * Submit data to Akismet service with unique PSForum User Agent
	 *
	 * This code is directly taken from the akismet_http_post() function and
	 * documented to PSForum 2.0 standard.
	 *
	 * @since PSForum (r3466)
	 *
	...
	private function http_post( $request, $host, $path, $port = 80, $ip = '' ) {
		...
		// Untque User Agent
		$akismet_ua     = "PSForum/{$psf_version} | ";
	...
	public function add_metaboxes() {
		...
		// Topics
		add_meta_box(
			'psf_akismet_topic_history',
			__( 'Akismet History', 'psforum' ),
	 ...
	 * @uses psf_time_since() To get the human readable time
	 */
	public function history_metabox() {
		...
		<div class="akismet-history" style="margin: 13px 0;">
							...
							<tr>
								<td style="color: #999; text-align: right; white-space: nowrap;">
									<span title="<?php echo esc_attr( date( 'D d M Y @ h:i:m a', $row['time'] ) . ' GMT' ); ?>">
										<?php psf_time_since( $row['time'], false, true ); ?>
									</span>
								</td>
			...
			<?php else : ?>
				...
				<p><?php esc_html_e( 'No recorded history. Akismet has not checked this post.', 'psforum' ); ?></p>
