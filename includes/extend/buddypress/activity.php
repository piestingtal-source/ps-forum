...
/**
 * PSForum BuddyPress Activity Class
 *
 * @package PSForum
...
if ( !class_exists( 'PSF_BuddyPress_Activity' ) ) :
/**
...
class PSF_BuddyPress_Activity {
	...
	/**
	 * The PSForum BuddyPress Activity loader
	 *
	 * @since PSForum (r3395)
	 */
	...
	private function setup_globals() {
		...
		// The name of the BuddyPress component, used in activity streams
		$this->component    = 'psforum';

		// Forums
		$this->forum_create = 'psf_forum_create';

		// Topics
		$this->topic_create = 'psf_topic_create';
		$this->topic_edit   = 'psf_topic_edit';
		$this->topic_close  = 'psf_topic_close';
		$this->topic_open   = 'psf_topic_open';

		// Replies
		$this->reply_create = 'psf_reply_create';
		$this->reply_edit   = 'psf_reply_edit';
	}
	...
	private function setup_actions() {
		...
		add_action( 'psf_new_topic',                     array( $this, 'topic_create'              ), 10, 4 );
		add_action( 'psf_new_reply',                     array( $this, 'reply_create'              ), 10, 5 );
		...
		add_action( 'psf_delete_topic',                  array( $this, 'topic_delete'              ), 10, 1 );
		add_action( 'psf_delete_reply',                  array( $this, 'reply_delete'              ), 10, 1 );
	...
	private function fully_loaded() {
		do_action_ref_array( 'psf_buddypress_activity_loaded', array( $this ) );
	}
	...
	public function register_activity_actions() {
		...
		bp_activity_set_action( $this->component, $this->topic_create, esc_html__( 'Neues Forumsthema', 'psforum' ) );
		bp_activity_set_action( $this->component, $this->reply_create, esc_html__( 'Neue Forumsantwort', 'psforum' ) );
	}

	/**
	 * Wrapper for recoding PSForum actions to the BuddyPress activity stream
	 *
	 * @since PSForum (r3395)
	...
	private function record_activity( $args = array() ) {
		...
		$activity = psf_parse_args( $args, array(
			'id'                => null,
	...
	/**
	 * Wrapper for deleting PSForum actions from BuddyPress activity stream
	 *
	 * @since PSForum (r3395)
	 * @param type $args Array of arguments for bp_activity_add()
	 * @uses psf_get_current_user_id()
	...
	private static function get_activity_id( $post_id = 0 ) {
		...
		$activity_id = (int) get_post_meta( $post_id, '_psf_activity_id', true );
	...
	?>
		...
		<option value="<?php echo $this->topic_create; ?>"><?php esc_html_e( 'Themen',  'psforum' ); ?></option>
		<option value="<?php echo $this->reply_create; ?>"><?php esc_html_e( 'Antworten', 'psforum' ); ?></option>
	 ...
	 * @uses psf_get_user_profile_link()
	 * @uses psf_get_topic_permalink()
	 * @uses psf_get_topic_title()
	 * @uses psf_get_topic_content()
	 * @uses psf_get_forum_permalink()
	 * @uses psf_get_forum_title()
	...
	public function topic_create( $topic_id = 0, $forum_id = 0, $anonymous_data = array(), $topic_author_id = 0 ) {
		...
		$user_link = psf_get_user_profile_link( $user_id  );

		// Topic
		$topic_permalink = psf_get_topic_permalink( $topic_id );
		$topic_title     = get_post_field( 'post_title',   $topic_id, 'raw' );
		$topic_content   = get_post_field( 'post_content', $topic_id, 'raw' );
		$topic_link      = '<a href="' . $topic_permalink . '">' . $topic_title . '</a>';

		// Forum
		$forum_permalink = psf_get_forum_permalink( $forum_id );
		$forum_title     = get_post_field( 'post_title', $forum_id, 'raw' );
		$forum_link      = '<a href="' . $forum_permalink . '">' . $forum_title . '</a>';

		// Activity action & text
		$activity_text    = sprintf( esc_html__( '%1$s hat das Thema %2$s im Forum %3$s gestartet', 'psforum' ), $user_link, $topic_link, $forum_link );
		$activity_action  = apply_filters( 'psf_activity_topic_create',         $activity_text, $user_id,   $topic_id,   $forum_id );
		$activity_content = apply_filters( 'psf_activity_topic_create_excerpt', $topic_content                                     );
		...
		$activity_id = $this->record_activity( array(
			...
			'hide_sitewide'     => ! psf_is_forum_public( $forum_id, false )
		) );

		// Add the activity entry ID as a meta value to the topic
		if ( !empty( $activity_id ) ) {
			update_post_meta( $topic_id, '_psf_activity_id', $activity_id );
		}
	}
	 ...
	 * @uses psf_get_public_status_id()
	 * @uses psf_get_closed_status_id()
	 * @uses psf_get_topic_forum_id()
	 * @uses psf_get_topic_author_id()
	...
	public function topic_update( $topic_id = 0, $post = null ) {
		...
		if ( get_post_type( $post ) !== psf_get_topic_post_type() )
			return;
		...
		if ( in_array( $post->post_status, array( psf_get_public_status_id(), psf_get_closed_status_id() ) ) ) {
	 ...
	 * @param array $anonymous_data
	 * @param int $topic_author_id
	 * @uses psf_get_reply_id()
	 * @uses psf_get_topic_id()
	 * @uses psf_get_forum_id()
	 * @uses psf_get_user_profile_link()
	 * @uses psf_get_reply_url()
	 * @uses psf_get_reply_content()
	 * @uses psf_get_topic_permalink()
	 * @uses psf_get_topic_title()
	 * @uses psf_get_forum_permalink()
	 * @uses psf_get_forum_title()
	...
	public function reply_create( $reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = array(), $reply_author_id = 0 ) {
		...
		// Activity action & text
		$activity_text    = sprintf( esc_html__( '%1$s hat auf das Thema %2$s im Forum %3$s geantwortet', 'psforum' ), $user_link, $topic_link, $forum_link );
		$activity_action  = apply_filters( 'psf_activity_reply_create',         $activity_text, $user_id, $reply_id,  $topic_id );
		$activity_content = apply_filters( 'psf_activity_reply_create_excerpt', $reply_content                                  );
		...
		if ( !empty( $activity_id ) ) {
			update_post_meta( $reply_id, '_psf_activity_id', $activity_id );
		}
	}
	...
	public function reply_update( $reply_id, $post ) {
		...
		if ( get_post_type( $post ) !== psf_get_reply_post_type() )
			return;
		...
		// Action based on new status
		if ( psf_get_public_status_id() === $post->post_status ) {

			// Validate reply data
			$topic_id        = psf_get_reply_topic_id( $reply_id );
			$forum_id        = psf_get_reply_forum_id( $reply_id );
			$reply_author_id = psf_get_reply_author_id( $reply_id );
