 ...
 * @since PSForum (r2593)
 *
 * @uses apply_filters() Calls 'psf_get_topic_caps' with the capabilities
 * @return array Topic capabilities
 */
function psf_get_topic_caps() {
	return apply_filters( 'psf_get_topic_caps', array (
 ...
 * @since PSForum (r2593)
 *
 * @uses apply_filters() Calls 'psf_get_topic_tag_caps' with the capabilities
 * @return array Topic tag capabilities
 */
function psf_get_topic_tag_caps() {
	return apply_filters( 'psf_get_topic_tag_caps', array (
 ...
 */
function psf_map_topic_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
	...
	switch ( $cap ) {
		...
		case 'read_topic' :
			...
			// Do some post ID based logic
				...
				if ( !empty( $_post ) ) {
					...
					// Post is public
					if ( psf_get_public_status_id() === $_post->post_status ) {
						$caps = array( 'spectate' );
		...
		case 'psf_topics_admin' :
			$caps = array( 'moderate' );
			break;
	}

	return apply_filters( 'psf_map_topic_meta_caps', $caps, $cap, $user_id, $args );
}
 ...
 * @return array Actual capabilities for meta capability
 */
function psf_map_topic_tag_meta_caps( $caps, $cap, $user_id, $args ) {
	...
	return apply_filters( 'psf_map_topic_tag_meta_caps', $caps, $cap, $user_id, $args );
}
