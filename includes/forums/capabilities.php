 ...
 * @since PSForum (r2593)
 *
 * @uses apply_filters() Calls 'psf_get_forum_caps' with the capabilities
 * @return array Forum capabilities
 */
function psf_get_forum_caps() {
	return apply_filters( 'psf_get_forum_caps', array (
 ...
 */
function psf_map_forum_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
	...
	switch ( $cap ) {
		...
		case 'psf_forums_admin' :
			$caps = array( 'keep_gate' );
			break;
	}

	return apply_filters( 'psf_map_forum_meta_caps', $caps, $cap, $user_id, $args );
}
