...
/**
 * Main PSF_Converter Class
 */
class PSF_Converter {

	/**
	 * The main PSForum Converter loader
	 *
	 * @since PSForum (r3813)
	 * @uses PSF_Converter::includes() Include the required files
	 * @uses PSF_Converter::setup_actions() Setup the actions
	 */
	public function __construct() {
		...
		switch ( strtoupper( $_SERVER['REQUEST_METHOD'] ) ) {
			...
			case 'GET'  :
				if ( ( empty( $_GET['page'] ) || ( 'psf-converter' !=  $_GET['page'] ) ) )
					return;
	...
	private function setup_actions() {
		...
		add_action( 'psf_register_admin_settings', array( $this, 'register_admin_settings' ) );
	...
	public function register_admin_settings() {
		...
		add_settings_section( 'psforum_converter_main',     __( 'Database Settings', 'psforum' ),  'psf_converter_setting_callback_main_section', 'psforum_converter' );

		// System Select
		add_settings_field( '_psf_converter_platform',      __( 'Select Platform',   'psforum' ),  'psf_converter_setting_callback_platform', 'psforum_converter', 'psforum_converter_main' );
		register_setting  ( 'psforum_converter_main',       '_psf_converter_platform',           'sanitize_title' );

		// Database Server
		add_settings_field( '_psf_converter_db_server',     __( 'Database Server',   'psforum' ),  'psf_converter_setting_callback_dbserver', 'psforum_converter', 'psforum_converter_main' );
		register_setting  ( 'psforum_converter_main',       '_psf_converter_db_server',          'sanitize_title' );

		// Database Server Port
		add_settings_field( '_psf_converter_db_port',       __( 'Database Port',     'psforum' ),  'psf_converter_setting_callback_dbport', 'psforum_converter', 'psforum_converter_main' );
		register_setting  ( 'psforum_converter_main',       '_psf_converter_db_port',            'sanitize_title' );

		// Database Name
		add_settings_field( '_psf_converter_db_name',       __( 'Database Name',     'psforum' ),  'psf_converter_setting_callback_dbname', 'psforum_converter', 'psforum_converter_main' );
		register_setting  ( 'psforum_converter_main',       '_psf_converter_db_name',            'sanitize_title' );

		// Database User
		add_settings_field( '_psf_converter_db_user',       __( 'Database User',     'psforum' ),  'psf_converter_setting_callback_dbuser', 'psforum_converter', 'psforum_converter_main' );
		register_setting  ( 'psforum_converter_main',       '_psf_converter_db_user',            'sanitize_title' );

		// Database Pass
		add_settings_field( '_psf_converter_db_pass',       __( 'Database Password', 'psforum' ),  'psf_converter_setting_callback_dbpass', 'psforum_converter', 'psforum_converter_main' );
		register_setting  ( 'psforum_converter_main',       '_psf_converter_db_pass',            'sanitize_title' );

		// Database Prefix
		add_settings_field( '_psf_converter_db_prefix',     __( 'Table Prefix',      'psforum' ),  'psf_converter_setting_callback_dbprefix', 'psforum_converter', 'psforum_converter_main' );
		register_setting  ( 'psforum_converter_main',       '_psf_converter_db_prefix',          'sanitize_title' );

		// Add the options section
		add_settings_section( 'psforum_converter_opt',      __( 'Options',           'psforum' ),  'psf_converter_setting_callback_options_section', 'psforum_converter' );

		// Rows Limit
		add_settings_field( '_psf_converter_rows',          __( 'Rows Limit',        'psforum' ),  'psf_converter_setting_callback_rows', 'psforum_converter', 'psforum_converter_opt' );
		register_setting  ( 'psforum_converter_opt',        '_psf_converter_rows',               'intval' );

		// Delay Time
		add_settings_field( '_psf_converter_delay_time',    __( 'Delay Time',        'psforum' ), 'psf_converter_setting_callback_delay_time', 'psforum_converter', 'psforum_converter_opt' );
		register_setting  ( 'psforum_converter_opt',        '_psf_converter_delay_time',        'intval' );

		// Convert Users ?
		add_settings_field( '_psf_converter_convert_users', __( 'Convert Users',     'psforum' ), 'psf_converter_setting_callback_convert_users', 'psforum_converter', 'psforum_converter_opt' );
		register_setting  ( 'psforum_converter_opt',        '_psf_converter_convert_users',     'intval' );

		// Restart
		add_settings_field( '_psf_converter_restart',       __( 'Start Over',        'psforum' ), 'psf_converter_setting_callback_restart', 'psforum_converter', 'psforum_converter_opt' );
		register_setting  ( 'psforum_converter_opt',        '_psf_converter_restart',           'intval' );

		// Clean
		add_settings_field( '_psf_converter_clean',         __( 'Purge Previous Import', 'psforum' ), 'psf_converter_setting_callback_clean', 'psforum_converter', 'psforum_converter_opt' );
		register_setting  ( 'psforum_converter_opt',        '_psf_converter_clean',             'intval' );
	}
	...
	public function admin_head() { ?>
		...
		<style type="text/css" media="screen">
			...
			div.psf-converter-updated,
			div.psf-converter-warning {
			...
			div.psf-converter-updated p.loading {
		...
		<script language="javascript">
			...
			function bbconverter_grab_data() {
				var values = {};
				jQuery.each(jQuery('#psf-converter-settings').serializeArray(), function(i, field) {
					values[field.name] = field.value;
				});

				if( values['_psf_converter_restart'] ) {
					jQuery('#_psf_converter_restart').removeAttr("checked");
				}

				if( values['_psf_converter_delay_time'] ) {
					bbconverter_delay_time = values['_psf_converter_delay_time'] * 1000;
				}

				values['action'] = 'bbconverter_process';
				values['_ajax_nonce'] = '<?php echo  wp_create_nonce( 'psf_converter_process' ); ?>';
			...
			function bbconverter_start() {
				if( false == bbconverter_is_running ) {
					...
					jQuery('#psf-converter-start').hide();
					jQuery('#psf-converter-stop').show();
					jQuery('#psf-converter-progress').show();
					bbconverter_log( '<p class="loading"><?php esc_html_e( 'Starting Conversion', 'psforum' ); ?></p>' );
					bbconverter_run();
				}
			}
			...
			function bbconverter_stop() {
				jQuery('#psf-converter-start').show();
				jQuery('#psf-converter-stop').hide();
				jQuery('#psf-converter-progress').hide();
				jQuery('#psf-converter-message p').removeClass( 'loading' );
			...
			function bbconverter_success(response) {
				...
				if ( response == '<p class="loading"><?php esc_html_e( 'Conversion Complete', 'psforum' ); ?></p>' || response.indexOf('error') > -1 ) {
					bbconverter_log('<p>Repair any missing information: <a href="<?php echo admin_url(); ?>tools.php?page=psf-repair">Continue</a></p>');
					bbconverter_stop();
			...
			function bbconverter_log(text) {
				if ( jQuery('#psf-converter-message').css('display') == 'none' ) {
					jQuery('#psf-converter-message').show();
				}
				if ( text ) {
					jQuery('#psf-converter-message p').removeClass( 'loading' );
					jQuery('#psf-converter-message').prepend( text );
				}
			}
	...
	private static function converter_output( $output = '' ) {
		...
		$after  = '</p>';
		$query  = get_option( '_psf_converter_query' );
	...
	public function process_callback() {
		...
		// Verify intent
		check_ajax_referer( 'psf_converter_process' );
		...
		if ( ! get_option( '_psf_converter_step' ) || ( !empty( $_POST['_psf_converter_restart'] ) ) ) {
			update_option( '_psf_converter_step',  1 );
			update_option( '_psf_converter_start', 0 );
		}

		$step  = (int) get_option( '_psf_converter_step',  1 );
		$min   = (int) get_option( '_psf_converter_start', 0 );
		$count = ! empty( $_POST['_psf_converter_rows'] ) ? min( max( (int) $_POST['_psf_converter_rows'], 1 ), 5000 ) : 100;
		$max   = ( $min + $count ) - 1;
		$start = $min;

		// Bail if platform did not get saved
		$platform = !empty( $_POST['_psf_converter_platform' ] ) ? sanitize_text_field( $_POST['_psf_converter_platform' ] ) : get_option( '_psf_converter_platform' );
		if ( empty( $platform ) )
			return;

		// Include the appropriate converter.
		$converter = psf_new_converter( $platform );

		switch ( $step ) {

			// STEP 1. Clean all tables.
			case 1 :
				if ( !empty( $_POST['_psf_converter_clean'] ) ) {
					if ( $converter->clean( $start ) ) {
						update_option( '_psf_converter_step',  $step + 1 );
						update_option( '_psf_converter_start', 0         );
						$this->sync_table( true );
						if ( empty( $start ) ) {
							$this->converter_output( __( 'No data to clean', 'psforum' ) );
						}
					} else {
						update_option( '_psf_converter_start', $max + 1 );
						$this->converter_output( sprintf( __( 'Deleting previously converted data (%1$s - %2$s)', 'psforum' ), $min, $max ) );
					}
				} else {
					update_option( '_psf_converter_step',  $step + 1 );
					update_option( '_psf_converter_start', 0         );
			...
			case 2 :
				if ( !empty( $_POST['_psf_converter_convert_users'] ) ) {
					if ( $converter->convert_users( $start ) ) {
						update_option( '_psf_converter_step',  $step + 1 );
						update_option( '_psf_converter_start', 0         );
						if ( empty( $start ) ) {
							$this->converter_output( __( 'No users to convert', 'psforum' ) );
						}
					} else {
						update_option( '_psf_converter_start', $max + 1 );
						$this->converter_output( sprintf(  __( 'Converting users (%1$s - %2$s)', 'psforum' ), $min, $max ) );
					}
				} else {
					update_option( '_psf_converter_step',  $step + 1 );
					update_option( '_psf_converter_start', 0         );
			...
			case 3 :
				if ( !empty( $_POST['_psf_converter_convert_users'] ) ) {
					if ( $converter->clean_passwords( $start ) ) {
						...
						update_option( '_psf_converter_start', 0         );
						if ( empty( $start ) ) {
							$this->converter_output( __( 'No passwords to clear', 'psforum' ) );
						}
					} else {
						update_option( '_psf_converter_start', $max + 1 );
						$this->converter_output( sprintf( __( 'Delete users WordPress default passwords (%1$s - %2$s)', 'psforum' ), $min, $max ) );
					}
				} else {
					update_option( '_psf_converter_step',  $step + 1 );
					update_option( '_psf_converter_start', 0         );
			...
			case 4 :
				if ( $converter->convert_forums( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No forums to convert', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Converting forums (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}
			...
			case 5 :
				if ( $converter->convert_forum_parents( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No forum parents to convert', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Calculating forum hierarchy (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}
			...
			case 6 :
				if ( $converter->convert_topics( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No topics to convert', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Converting topics (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}
			...
			case 7 :
				if ( $converter->convert_topic_stickies( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No stickies to stick', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Calculating topic stickies (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}
			...
			case 8 :
				if ( $converter->convert_topic_super_stickies( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No super stickies to stick', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Calculating topic super stickies (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}
			...
			case 9 :
				if ( $converter->convert_tags( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No tags to convert', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Converting topic tags (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}
			...
			case 10 :
				if ( $converter->convert_replies( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No replies to convert', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Converting replies (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}
			...
			case 11 :
				if ( $converter->convert_reply_to_parents( $start ) ) {
					...
					update_option( '_psf_converter_start', 0         );
					if ( empty( $start ) ) {
						$this->converter_output( __( 'No reply_to parents to convert', 'psforum' ) );
					}
				} else {
					update_option( '_psf_converter_start', $max + 1 );
					$this->converter_output( sprintf( __( 'Calculating reply_to parents (%1$s - %2$s)', 'psforum' ), $min, $max ) );
				}

				break;

			default :
				delete_option( '_psf_converter_step'  );
				delete_option( '_psf_converter_start' );
				delete_option( '_psf_converter_query' );

				$this->converter_output( __( 'Conversion Complete', 'psforum' ) );
 ...
 * @since PSForum (r3813)
 */
abstract class PSF_Converter_Base {
	...
	private function setup_globals() {
		...
		$this->wpdb     = $wpdb;
		$this->max_rows = ! empty( $_POST['_psf_converter_rows'] )
			? min( max( (int) $_POST['_psf_converter_rows'], 1 ), 5000 )
			: 100;

		$this->opdb = new wpdb(
			sanitize_text_field( $_POST['_psf_converter_db_user']   ),
			sanitize_text_field( $_POST['_psf_converter_db_pass']   ),
			sanitize_text_field( $_POST['_psf_converter_db_name']   ),
			sanitize_text_field( $_POST['_psf_converter_db_server'] )
		);

		$this->opdb->prefix = sanitize_text_field( $_POST['_psf_converter_db_prefix'] );
		 ...
		 * Syncing
		 */
		$this->sync_table_name = $this->wpdb->prefix . 'psf_converter_translator';
	...
	public function convert_table( $to_type, $start ) {
		...
		if ( !empty( $from_tablename ) ) {
			...
			update_option( '_psf_converter_query', $forum_query );
						...
						if ( ( $row['to_type'] == $to_type ) && ! is_null( $row['to_fieldname'] ) ) {
							...
							if ( in_array( $row['to_fieldname'], $tablefield_array ) ) {
								...
								// Translates a field from the old forum.
									...
									if ( ( 'callback_userid' == $row['callback_method'] ) && empty( $_POST['_psf_converter_convert_users'] ) ) {
							...
							// to do some extra work or set a default.
								...
								// Translates a field from the old forum.
									...
									if ( ( $row['callback_method'] == 'callback_userid' ) && ( 0 == $_POST['_psf_converter_convert_users'] ) ) {
							...
							default:
								...
								if ( is_numeric( $post_id ) ) {
									...
									foreach ( $insert_postmeta as $key => $value ) {
										...
										if ( ( 'forum' == $to_type ) && ( '_psf_forum_id' == $key ) )
											add_post_meta( $post_id, '_psf_old_forum_id', $value );

										// Topics need an extra bit of metadata
										// to be keyed to the new post_id
										if ( ( 'topic' == $to_type ) && ( '_psf_topic_id' == $key ) ) {

											// Update the live topic ID
											update_post_meta( $post_id, $key, $post_id );

											// Save the old topic ID
											add_post_meta( $post_id, '_psf_old_topic_id', $value );
											if ( '_id' == substr( $key, -3 ) && ( true === $this->sync_table ) ) {
												$this->wpdb->insert( $this->sync_table_name, array( 'value_type' => 'post', 'value_id' => $post_id, 'meta_key' => '_psf_topic_id',     'meta_value' => $post_id ) );
												$this->wpdb->insert( $this->sync_table_name, array( 'value_type' => 'post', 'value_id' => $post_id, 'meta_key' => '_psf_old_topic_id', 'meta_value' => $value   ) );
											}
										...
										if ( ( 'reply' == $to_type ) && ( '_psf_reply_to' == $key ) ) {
											add_post_meta( $post_id, '_psf_old_reply_to', $value );
										}
									}
								}
								break;
	...
	/**
	 * This method converts old forum heirarchy to new PSForum heirarchy.
	 */
	public function convert_forum_parents( $start ) {
		...
		if ( !empty( $this->sync_table ) ) {
			$query = 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_forum_parent_id" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query = 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_psf_forum_parent_id" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		}

		update_option( '_psf_converter_query', $query );
	...
	/**
	 * This method converts old topic stickies to new PSForum stickies.
	 *
	 * @since PSForum (r)
	 *
	 * @uses WPDB $wpdb
	 * @uses psf_stick_topic() to set the imported topic as sticky
	 *
	 */
	public function convert_topic_stickies( $start ) {
		...
		if ( !empty( $this->sync_table ) ) {
			$query = 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_old_sticky_status" AND meta_value = "sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query = 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_psf_old_sticky_status" AND meta_value = "sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		}
	...
	/**
	 * This method converts old topic super stickies to new PSForum super stickies.
	 *
	 * @since PSForum (r)
	 *
	 * @uses WPDB $wpdb
	 * @uses psf_stick_topic() to set the imported topic as super sticky
	 *
	 */
	public function convert_topic_super_stickies( $start ) {
		...
		if ( !empty( $this->sync_table ) ) {
			$query = 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_old_sticky_status" AND meta_value = "super-sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query = 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_psf_old_sticky_status" AND meta_value = "super-sticky" LIMIT ' . $start . ', ' . $this->max_rows;
		}
		...
		foreach ( (array) $sticky_array as $row ) {
			$super = true;
			psf_stick_topic( $row->value_id, $super );
			$has_update = true;
		}
	...
	/**
	 * This method converts old reply_to post id to new PSForum reply_to post id.
	 */
	public function convert_reply_to_parents( $start ) {
		...
		if ( !empty( $this->sync_table ) ) {
			$query = 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_old_reply_to" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		} else {
			$query = 'SELECT post_id AS value_id, meta_value FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_psf_old_reply_to" AND meta_value > 0 LIMIT ' . $start . ', ' . $this->max_rows;
		}
		...
		foreach ( (array) $reply_to_array as $row ) {
			...
			$this->wpdb->query( 'UPDATE ' . $this->wpdb->postmeta . ' SET meta_value = "' . $reply_to . '" WHERE meta_key = "_psf_reply_to" AND post_id = "' . $row->value_id . '" LIMIT 1' );
			$has_update = true;
		}
	...
	public function clean( $start ) {
		...
		if ( true === $this->sync_table ) {
			$query = 'SELECT value_id FROM ' . $this->sync_table_name . ' INNER JOIN ' . $this->wpdb->posts . ' ON(value_id = ID) WHERE meta_key LIKE "_psf_%" AND value_type = "post" GROUP BY value_id ORDER BY value_id DESC LIMIT ' . $this->max_rows;
		} else {
			$query = 'SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key LIKE "_psf_%" GROUP BY post_id ORDER BY post_id DESC LIMIT ' . $this->max_rows;
		}
		...
		if ( true === $this->sync_table ) {
			$query = 'SELECT value_id FROM ' . $this->sync_table_name . ' INNER JOIN ' . $this->wpdb->users . ' ON(value_id = ID) WHERE meta_key = "_psf_user_id" AND value_type = "user" LIMIT ' . $this->max_rows;
		} else {
			$query = 'SELECT user_id AS value_id FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_psf_user_id" LIMIT ' . $this->max_rows;
		}
	...
	public function clean_passwords( $start ) {
		...
		$query       = 'SELECT user_id, meta_value FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_psf_password" LIMIT ' . $start . ', ' . $this->max_rows;
		update_option( '_psf_converter_query', $query );
		...
		if ( !empty( $bbconverter ) ) {
			...
			foreach ( $bbconverter as $value ) {
				if ( is_serialized( $value['meta_value'] ) ) {
					...
					$this->wpdb->query( 'DELETE FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_psf_password" AND user_id = "' . $value['user_id'] . '"' );
				}
			}
			$has_delete = true;
		}
	...
	public function callback_pass( $username, $password ) {
		...
		if ( !empty( $user ) ) {
			$usermeta = $this->wpdb->get_row( 'SELECT * FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_psf_password" AND user_id = "' . $user->ID . '" LIMIT 1' );
			...
			if ( !empty( $usermeta ) ) {
				if ( $this->authenticate_pass( $password, $usermeta->meta_value ) ) {
					...
					$this->wpdb->query( 'DELETE FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_psf_password" AND user_id = "' . $user->ID . '"' );
				}
			}
		}
	}
	...
	private function callback_forumid( $field ) {
		if ( !isset( $this->map_forumid[$field] ) ) {
			if ( !empty( $this->sync_table ) ) {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_forum_id" AND meta_value = "%s" LIMIT 1', $field ) );
			} else {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_psf_forum_id" AND meta_value = "%s" LIMIT 1', $field ) );
			}
	...
	private function callback_topicid( $field ) {
		if ( !isset( $this->map_topicid[$field] ) ) {
			if ( !empty( $this->sync_table ) ) {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_old_topic_id" AND meta_value = "%s" LIMIT 1', $field ) );
			} else {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_psf_old_topic_id" AND meta_value = "%s" LIMIT 1', $field ) );
			}
	...
	private function callback_reply_to( $field ) {
		if ( !isset( $this->map_reply_to[$field] ) ) {
			if ( !empty( $this->sync_table ) ) {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_post_id" AND meta_value = "%s" LIMIT 1', $field ) );
			} else {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT post_id AS value_id FROM ' . $this->wpdb->postmeta . ' WHERE meta_key = "_psf_post_id" AND meta_value = "%s" LIMIT 1', $field ) );
			}
	...
	private function callback_userid( $field ) {
		if ( !isset( $this->map_userid[$field] ) ) {
			if ( !empty( $this->sync_table ) ) {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT value_id, meta_value FROM ' . $this->sync_table_name . ' WHERE meta_key = "_psf_user_id" AND meta_value = "%s" LIMIT 1', $field ) );
			} else {
				$row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT user_id AS value_id FROM ' . $this->wpdb->usermeta . ' WHERE meta_key = "_psf_user_id" AND meta_value = "%s" LIMIT 1', $field ) );
			}

			if ( !is_null( $row ) ) {
				$this->map_userid[$field] = $row->value_id;
			} else {
				if ( !empty( $_POST['_psf_converter_convert_users'] ) && ( $_POST['_psf_converter_convert_users'] == 1 ) ) {
	...
	protected function callback_html( $field ) {
		require_once( psforum()->admin->admin_dir . 'parser.php' );
 ...
 */
function psf_new_converter( $platform ) {
	$found = false;

	if ( $curdir = opendir( psforum()->admin->admin_dir . 'converters/' ) ) {
	...
	if ( true === $found ) {
		require_once( psforum()->admin->admin_dir . 'converters/' . $platform . '.php' );
		return new $platform;
	} else {
		return null;
	}
