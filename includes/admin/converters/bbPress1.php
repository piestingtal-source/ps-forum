...
/**
 * PSForum 1.1 Converter
 *
 * @since PSForum (r3816)
 * @link Codex Docs http://codex.psforum.org/import-forums/psforum-1-x-buddypress-group-forums
 */
class PSForum1 extends PSF_Converter_Base {

	/**
	 * Main constructor
	 *
	 * @uses PSForum1::setup_globals()
	 */
	...
	public function setup_globals() {
		...
		$this->field_map[] = array(
			...
			'to_type'        => 'forum',
			'to_fieldname'   => '_psf_forum_parent_id'
		);
		...
		$this->field_map[] = array(
			...
			'from_fieldname' => 'topics',
			'to_type'        => 'forum',
			'to_fieldname'   => '_psf_total_topic_count'
		);
		...
		$this->field_map[] = array(
			...
			'from_fieldname' => 'posts',
			'to_type'        => 'forum',
			'to_fieldname'   => '_psf_total_reply_count'
		);
		...
		// Forum type (PSForum v1.x Forum > 0 or Category = 0, Stored in postmeta)
		$this->field_map[] = array(
			'from_tablename'  => 'meta',
		...
		// Topic status (Spam, Trash or Publish, PSForum v1.x publish = 0, trash = 1 & spam = 2)
		$this->field_map[] = array(
			'from_tablename'  => 'posts',
		...
		$this->field_map[] = array(
			...
			'to_type'         => 'topic',
			'to_fieldname'    => '_psf_old_sticky_status',
		...
		$this->field_map[] = array(
			...
			'from_fieldname' => 'topic_time',
			'to_type'        => 'topic',
			'to_fieldname'   => '_psf_last_active_time'
		);
		...
		// Reply status (Spam, Trash or Publish, PSForum v1.x publish = 0, trash = 1 & spam = 2)
		$this->field_map[] = array(
			'from_tablename'  => 'posts',
			'from_fieldname'  => 'post_status',
			'to_type'         => 'reply',
	...
	/**
	 * Translate the post status from PSForum 1's numeric's to WordPress's
	 * strings.
	 *
	 * @param int $status PSForum 1.x numeric post status
	 * @return string WordPress safe
	 */
	public function callback_status( $status = 0 ) {
		switch ( $status ) {
			case 2 :
				$status = 'spam';    // psf_get_spam_status_id()
				break;

			case 1 :
				$status = 'trash';   // psf_get_trash_status_id()
				break;

			case 0  :
			default :
				$status = 'publish'; // psf_get_public_status_id()
				break;
		}
		return $status;
	}
	...
	/**
	 * Translate the forum type from PSForum 1.x numeric's to WordPress's strings.
	 *
	 * @param int $status PSForum 1.x numeric forum type
	 * @return string WordPress safe
	 */
	...
	/**
	 * Translate the topic sticky status type from PSForum 1.x numeric's to WordPress's strings.
	 *
	 * @param int $status PSForum 1.x numeric forum type
	 * @return string WordPress safe
	 */
	public function callback_sticky_status( $status = 0 ) {
		switch ( $status ) {
			case 2 :
				$status = 'super-sticky'; // PSForum Super Sticky 'topic_sticky = 2'
				break;

			case 1 :
				$status = 'sticky';       // PSForum Sticky 'topic_sticky = 1'
				break;

			case 0  :
			default :
				$status = 'normal';       // PSForum Normal Topic 'topic_sticky = 0'
				break;
		}
		return $status;
	}
	 ...
	 *
	 * @param int $count PSForum 1.x topic and reply counts
	 * @return string WordPress safe
	 */
	 ...
	 * Set the reply title
	 *
	 * @param string $title PSForum 1.x topic title of this reply
	 * @return string Prefixed topic title, or empty string
	 */
	public function callback_reply_title( $title = '' ) {
		$title = !empty( $title ) ? __( 'Re: ', 'psforum' ) . html_entity_decode( $title ) : '';
		return $title;
	}
